...
async function checkVideoAccessibility(html) {
    try {
        ...
        if (videoElement.length > 0) {
            ...
            const transcriptLinkBefore = videoElement.prev('a[href*="transcript"]');
            const transcriptLinkAfter = videoElement.next('a[href*="transcript"]');
            const transcriptLink = transcriptLinkBefore.length > 0 ? transcriptLinkBefore : transcriptLinkAfter;
            ...
            if (transcriptContent.trim() === videoTranscription.trim()) {
                ...
                if (audioElements.length > 0) {
                    audioElements.each(async (index, audioElement) => {
                        const audioTranscriptLinkBefore = $(audioElement).prev('a[href*="transcript"]');
                        const audioTranscriptLinkAfter = $(audioElement).next('a[href*="transcript"]');
                        const audioTranscriptLink = audioTranscriptLinkBefore.length > 0 ? audioTranscriptLinkBefore : audioTranscriptLinkAfter;
...
async function checkAudioAccessibility(html) {
    try {
        ...
        if (audioElement.length > 0) {
            // Fetch transcript content if link found
            const transcriptLinkBefore = audioElement.prev('a[href*="transcript"]');
            const transcriptLinkAfter = audioElement.next('a[href*="transcript"]');
            const transcriptLink = transcriptLinkBefore.length > 0 ? transcriptLinkBefore : transcriptLinkAfter;
            if (transcriptLink.length === 0) {
