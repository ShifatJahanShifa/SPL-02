...
function Output1() {
    ...
    const fetchAndInsertLogs = async () => {
        try {
            const urlToInsert = window.location.href; // Current page URL
            
            // Insert the URL
            await insertUrlName(urlToInsert);
            ...
            console.error('Error fetching logs and inserting URL:', error);
            setLoading(false);
        }
    };

    const insertUrlName = async (urlName) => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/insertUrl`, { urlName });
            console.log('URL insertion response:', response.data);
        } catch (error) {
